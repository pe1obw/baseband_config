...
def handle_sdr_commands(sdr : Sdr, args):
    ...
    if args.set_default:
        sdr.set_default()
        sdr.dump_settings(sdr.read_settings())
        print('Default settings loaded to SDR')

    if args.info:
        print('\nActual SDR settings:')
        settings = sdr.read_settings()
        sdr.dump_settings(settings)

    if args.set is not None:
        settings = sdr.read_settings()
        for setting in args.set:
            param, value = setting.split('=')
            sdr.set_using_name_value(settings, param, value)
        sdr.dump_settings(settings)
        sdr.write_settings(settings)

    if args.settings_to_file:
        settings = sdr.read_settings()
        sdr.dump_settings(settings)
        with open(args.settings_to_file, 'w') as file:
            json.dump(sdr.serialize(settings), file, indent=4)
        print(f'Settings read from SDR and written to {args.settings_to_file}')

    if args.settings_from_file:
        with open(args.settings_from_file, 'r') as file:
            current_settings = sdr.read_settings()
            serialized_settings = json.load(file)
            settings = sdr.deserialize(serialized_settings, SDR_SETTINGS, current_settings)
            sdr.write_settings(settings)
            sdr.dump_settings(settings)
...
def handle_baseband_commands(bb : Baseband, args):
    ...
    if args.settings_from_file:
        with open(args.settings_from_file, 'r') as file:
            ...
            serialized_settings = json.load(file)
            settings = bb.deserialize(serialized_settings, BB_SETTINGS, current_settings)
...
def main():
    ...
    # Common commands
    parser.add_argument('--sdr', action='store_true', help='Access SDR')
    parser.add_argument('--bb', action='store_true', help='Access baseband')
    parser.add_argument('--pulse_gpio', type=int, help=f'Pulse GPIO pin <n> for {GPIO_PULSE_LENGTH} seconds')

    # Baseband commands
    parser.add_argument('--info', action='store_true', help='Read device info (BB and SDR)')
    parser.add_argument('--set', action='append', type=str, help='Set a setting (e.g. --set video.video_mode=PAL (BB and SDR))')
    parser.add_argument('--set_default', action='store_true', help='Set actual settings to default (BB and SDR)')
    parser.add_argument('--settings_to_file', type=str, help='Store actual settings to file (BB and SDR)')
    parser.add_argument('--settings_from_file', type=str, help='Read settings from file and write to baseband (BB and SDR)')
    ...
    parser.add_argument('--load_preset', type=int, help=f'Load preset <n> (1..{NR_PRESETS-1})')
    parser.add_argument('--store_preset', type=int, help=f'Store actual settings to preset <n> (1..{NR_PRESETS-1})')
    parser.add_argument('--erase_preset', type=int, help=f'Erase preset <n> (1..{NR_PRESETS-1})')
    ...
    if args.sdr:
        sdr = Sdr(usb_driver)
        usb_driver.slave_addr = SDR_I2C_SLAVE_ADDRESS
