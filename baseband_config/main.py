...
try:
    ...
    pass
import time
from baseband.baseband import NR_PRESETS, Baseband, BB_I2C_SLAVE_ADDRESS
...
def main():
    ...
    if args.sdr:
        ...
        settings.frequency_khz = 1000000
        settings.gain_db = 0
        # settings.bb_gain = 16384  # 1.0x
        settings.bw_mhz = 40
        settings.enable = 1
        settings.fir_filter_mhz = 50
        sdr.write_settings(settings)

        settings = sdr.read_settings()
        sdr.dump_settings(settings)
        usb_driver.slave_addr = BB_I2C_SLAVE_ADDRESS  # Restore slave address for baseband
    ...
    if args.bb:
        ...
        if not (int(major) >= 2):
            print('Baseband Firmware version 2.0 or higher required')
            exit(0)
